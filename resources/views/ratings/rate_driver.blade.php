    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <div class="bg-yellow-500 text-white px-6 py-4">
                <h2 class="text-xl font-bold">Rate Driver</h2>
            </div>
                ...
                <form action="{{ route('ratings.store') }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea class="w-full border-gray-300 rounded-md shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-200" 
                            id="comment" name="comment" rows="3" placeholder="Tell us about your experience with this driver...">{{ old('comment') }}</textarea>
                        @error('comment')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Update stars appearance based on rating
